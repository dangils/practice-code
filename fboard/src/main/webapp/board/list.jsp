...
%>	
	...
	<jsp:useBean id="replyDTO" class="com.koreait.reply.ReplyDTO"/>
...
<%
		...
		//List<BoardDTO> pagelist = boardDAO.page(start,pagePerCount);
		//자바빈즈로 선언한 boardDTO객체에 boardDAO.page 메소드 리턴값을 셋함
		//이때, page 리턴값, Dtolist 둘다 Arraylist 형식
		
		if(boardDAO.page(start,pagePerCount)!=null){
		for(BoardDTO list : boardDAO.page(start,pagePerCount)){
  ...
  //BoardDTO 형식을 가지는 Dtolist(arraylist 타입)의 각 객체들을 하나씩 가져와 각 객체별로 아래 기능 수행
		...
		int idx = list.getIdx();
		//향상된 포문에서의 list변수(BoardDTO에서 getIdx를 겟함)
...
%>
		<tr>
			<td><%=list.getIdx()%></td>
			<td><a href="./view.jsp?b_idx=<%=list.getIdx()%>"><%=list.getTitle()%></a> <%=boardDAO.reply(idx) %>  <%=newDateStr%>
			<%=imgCheck%>
